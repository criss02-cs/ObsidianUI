...
public class EventCollection : Dictionary<DateTime, ICollection>
{
    ...
    /// This method returns false if key is not found in the <see cref="System.Collections.Generic.Dictionary"/>
    /// </returns>
    public new bool Remove(DateTime key)
    {
        ...
        if (removed)
        {
            CollectionChanged?.Invoke(this, new EventCollectionChangedArgs
            {
                Item = key.Date,
                Type = EventCollectionChangedType.Remove
            });
        }
        return removed;
    }
